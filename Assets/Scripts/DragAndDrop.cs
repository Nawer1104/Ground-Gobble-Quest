...
public class DragAndDrop : MonoBehaviour
{
    ...
    [SerializeField] GameObject vfxOnSuccess;
    [SerializeField] GameObject vfxOnDeath;
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "destroyable")
        {
            ...
            GameObject explosion = Instantiate(vfxOnSuccess, collision.gameObject.transform.position, transform.rotation);
            Destroy(explosion, .75f);
            Destroy(collision.gameObject);
        }
        else if (collision.gameObject.tag == "people")
        {
            ResetPos();
